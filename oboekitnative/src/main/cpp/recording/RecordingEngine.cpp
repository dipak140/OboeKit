    ...
    if (a == oboe::StreamState::Started) {
        ...
        while (isRecording) {
            ...
            if (result == oboe::Result::OK) {
                ...
                pauseRecording();  // Stop the current stream
                resumeRecording(); // Attempt to reopen and start the stream
                if (stream == nullptr || stream->getState() != oboe::StreamState::Started) {
                    __android_log_print(ANDROID_LOG_ERROR, "OboeAudioRecorder", "Failed to resume recording after disconnection");
                    isRecording = false; // Stop recording if resumption fails
                    break;
                }
            }
        }
...
void RecordingEngine::resumeRecording() {
    ...
    std::ofstream f;
    f.open(this->path, std::ios::binary | std::ios::app); // Open in append mode
